...
<div class="container">
  ...
  <div class="quizzes-panel <?php echo $active_tab === 1 ? 'active' : ''; ?>" id="panel1">
    ...
    <div class="game-area">
      ...
      <div class="controls">
        <button class="btn-action btn-secondary" onclick="m_nextRound()"><i class="fas fa-forward"></i> <span id="p1SkipLbl">Next Round</span></button>
      </div>
    </div>
  </div>
  ...
  <div class="quizzes-panel <?php echo $active_tab === 4 ? 'active' : ''; ?>" id="panel4">
    ...
    <div class="quiz-question-card" id="p4QCard">
      ...
      <div style="text-align:center"><div class="q-badge-pill" id="p4QBadge"><i class="fas fa-paw"></i> Animal Question</div></div>
    </div>
...
function applyAllUI() {
  // UI strings per panel are set inside each start function
}
...
const M_COPY={en:{title:'Animal Name Match!',sub:'Match each animal name to the correct picture!',colName:'Animal Names',colPhoto:'Animal Photos',roundInfo:r=>`Round ${r} — Match 4 pairs!`,progressLbl:(m,t)=>`${m} / ${t} matched`,skipLbl:'Next Round',feedbackCorrect:n=>`✅ "${n}" — correct match!`,feedbackWrong:'❌ Not quite! Try again!',resultTitle:'Amazing!',resultSub:'You matched all the animals!'},tl:{title:'Pagtutugma ng Hayop!',sub:'Itugma ang bawat pangalan ng hayop sa tamang larawan!',colName:'Mga Pangalan',colPhoto:'Mga Larawan',roundInfo:r=>`Round ${r} — Itugma ang 4 pares!`,progressLbl:(m,t)=>`${m} / ${t} natugma`,skipLbl:'Susunod na Round',feedbackCorrect:n=>`✅ "${n}" — tama ang tugma!`,feedbackWrong:'❌ Hindi pa! Subukan ulit!',resultTitle:'Kahanga-hanga!',resultSub:'Natugma mo ang lahat ng hayop!'}};
const M_TOTAL_ROUNDS=3, M_PPM=10, M_MAX=M_TOTAL_ROUNDS*4*M_PPM;
let m_score=0,m_round=0,m_matched=0,m_pool=[],m_roundAnimals=[],m_selName=null,m_selPhoto=null;
...
function m_startGame(){
  ...
  document.getElementById('p1SkipLbl').textContent=c.skipLbl;
  saveStart(ACT_IDS[1]);
  m_nextRound();
}
function m_nextRound(){
  m_round++;m_matched=0;m_selName=null;m_selPhoto=null;
...
function m_selectName(card,animal){if(card.classList.contains('matched'))return;document.querySelectorAll('#p1NameCol .name-card.selected').forEach(c=>c.classList.remove('selected'));m_selName={card,animal};card.classList.add('selected');if(m_selName&&m_selPhoto)m_checkMatch();}
function m_selectPhoto(card,animal){if(card.classList.contains('matched'))return;document.querySelectorAll('#p1PhotoCol .photo-card.selected').forEach(c=>c.classList.remove('selected'));m_selPhoto={card,animal};card.classList.add('selected');if(m_selName&&m_selPhoto)m_checkMatch();}
function m_checkMatch(){
  const n=m_selName,p=m_selPhoto;m_selName=null;m_selPhoto=null;
  const c=M_COPY[lang];
  if(n.animal.name===p.animal.name){
    ...
    m_score+=M_PPM; saveProgress(ACT_IDS[1],Math.round((m_score/M_MAX)*100));m_matched++;
    document.getElementById('p1Score').textContent=m_score;
    document.getElementById('p1Bar').style.width=((m_matched/4)*100)+'%';
    document.getElementById('p1BarLbl').textContent=c.progressLbl(m_matched,4);
    document.getElementById('p1Feedback').textContent=c.feedbackCorrect(n.animal.name);
    document.getElementById('p1Feedback').className='feedback-msg correct';
    confetti(CAT_COLORS[n.animal.cat]||'#A0522D');speak(n.animal.name,lang);
    if(m_matched===4){setTimeout(()=>{if(m_round>=M_TOTAL_ROUNDS)m_showResult();else m_nextRound();},900);}
  } else {
    n.card.classList.add('wrong');p.card.classList.add('wrong');
    ...
    setTimeout(()=>{n.card.classList.remove('wrong','selected');p.card.classList.remove('wrong','selected');},700);
  }
}
...
const S_COPY={en:{title:'Spot the Animal!',sub:'Read the name and find the correct animal!',prompt:'Find this animal:',next:'Next',feedbackCorrect:n=>`✅ Yes! That's the ${n}!`,feedbackWrong:n=>`❌ That's the ${n}! Keep going!`,resultTitle:'Superstar!',resultSub:'You found all the animals!'},tl:{title:'Hanapin ang Hayop!',sub:'Basahin ang pangalan at hanapin ang tamang hayop!',prompt:'Hanapin ang hayop na ito:',next:'Susunod',feedbackCorrect:n=>`✅ Tama! Iyan ang ${n}!`,feedbackWrong:n=>`❌ Iyan ang ${n}! Magpatuloy!`,resultTitle:'Mahusay!',resultSub:'Nahanap mo ang lahat ng hayop!'}};
const S_TOTAL=10,S_MAX=S_TOTAL*10;
let s_pool=[],s_current=null,s_answered=false,s_score=0,s_qIdx=0,s_lives=3;

function s_startGame(){
  s_score=0;s_qIdx=0;s_lives=3;s_answered=false;s_pool=shuffle([...S_ANIMALS[lang]]);
...
function s_loadQuestion(){
  ...
  choices.forEach(a=>{
    ...
    card.onclick=()=>s_checkAnswer(card,a,isCorrect);
    grid.appendChild(card);
  });
  setTimeout(s_speakAnimal,400);
}
function s_speakAnimal(){if(s_current)speak(s_current.name,lang);}
function s_checkAnswer(card,animal,isCorrect){
  if(s_answered)return;s_answered=true;
  document.querySelectorAll('#p2Choices .choice-card').forEach(c=>c.classList.add('answered'));
  const c=S_COPY[lang];
  if(isCorrect){
    card.classList.add('correct');s_score+=10; saveProgress(ACT_IDS[2],Math.round((s_score/S_MAX)*100));document.getElementById('p2Score').textContent=s_score;
    ...
    document.getElementById('p2Feedback').textContent=c.feedbackWrong(animal.name);
  ...
  if(s_qIdx>=S_TOTAL||s_lives<=0){
    ...
    // Auto-next after 1 second — no need to press button
    setTimeout(s_loadQuestion, 1000);
  }
}
...
const SO_ANIMALS={
  ...
  {name:'Cat',emoji:'🐈',cat:'land',img:'pictures/animals/cat.jpg'},
  {name:'Horse',emoji:'🐴',cat:'land',img:'pictures/animals/horse.jpg'},
  {name:'Pig',emoji:'🐷',cat:'land',img:'pictures/animals/pig.jpg'},
  {name:'Rabbit',emoji:'🐰',cat:'land',img:'pictures/animals/rabbit.jpg'},
  {name:'Eagle',emoji:'🦅',cat:'air',img:'pictures/animals/eagle.jpg'},
  {name:'Parrot',emoji:'🦜',cat:'air',img:'pictures/animals/parrot.jpg'},
  {name:'Owl',emoji:'🦉',cat:'air',img:'pictures/animals/owl.jpg'},
  {name:'Bat',emoji:'🦇',cat:'air',img:'pictures/animals/bat.jpg'},
  {name:'Fish',emoji:'🐠',cat:'water',img:'pictures/animals/fishh.jpg'},
  {name:'Dolphin',emoji:'🐬',cat:'water',img:'pictures/animals/dolphin.jpg'},
  {name:'Turtle',emoji:'🐢',cat:'water',img:'pictures/animals/turtle.jpg'}],

  tl:[
  {name:'Aso',emoji:'🐕',cat:'land',img:'pictures/animals/dogi.jpg'},
  {name:'Pusa',emoji:'🐈',cat:'land',img:'pictures/animals/cat.jpg'},
  {name:'Kabayo',emoji:'🐴',cat:'land',img:'pictures/animals/horse.jpg'},
  {name:'Baboy',emoji:'🐷',cat:'land',img:'pictures/animals/pig.jpg'},
  {name:'Kuneho',emoji:'🐰',cat:'land',img:'pictures/animals/rabbit.jpg'},
  {name:'Agila',emoji:'🦅',cat:'air',img:'pictures/animals/eagle.jpg'},
  {name:'Loro',emoji:'🦜',cat:'air',img:'pictures/animals/parrot.jpg'},
  {name:'Kuwago',emoji:'🦉',cat:'air',img:'pictures/animals/owl.jpg'},
  {name:'Paniki',emoji:'🦇',cat:'air',img:'pictures/animals/bat.jpg'},
  {name:'Isda',emoji:'🐠',cat:'water',img:'pictures/animals/fishh.jpg'},
  {name:'Lumba-lumba',emoji:'🐬',cat:'water',img:'pictures/animals/dolphin.jpg'},
  {name:'Pagong',emoji:'🐢',cat:'water',img:'pictures/animals/turtle.jpg'}]
};

const SO_COPY={en:{title:'Animal Sorting!',sub:'Where does this animal live? Sort them!',prompt:'Where does this animal live?',land:'Land',landDesc:'Ground animals',air:'Air',airDesc:'Flying animals',water:'Water',waterDesc:'Ocean/water animals',feedbackCorrect:(n,c)=>`✅ Right! ${n} lives on ${c}!`,feedbackWrong:(n,c)=>`❌ ${n} actually lives on ${c}!`,resultTitle:'Great Sorting!',resultSub:'You sorted all the animals!'},tl:{title:'Pag-aayos ng Hayop!',sub:'Saan nakatira ang hayop na ito? Ayusin sila!',prompt:'Saan nakatira ang hayop na ito?',land:'Lupa',landDesc:'Mga hayop sa lupa',air:'Hangin',airDesc:'Mga hayop na lumilipad',water:'Tubig',waterDesc:'Mga hayop sa tubig',feedbackCorrect:(n,c)=>`✅ Tama! Ang ${n} ay nakatira sa ${c}!`,feedbackWrong:(n,c)=>`❌ Ang ${n} ay nakatira sa ${c}!`,resultTitle:'Napakahusay!',resultSub:'Nayos mo ang lahat ng hayop!'}};
const SO_TOTAL=12,SO_MAX=SO_TOTAL*10;
let so_pool=[],so_current=null,so_answered=false,so_score=0,so_qIdx=0;

function so_startGame(){
  so_score=0;so_qIdx=0;so_answered=false;so_pool=shuffle([...SO_ANIMALS[lang]]);
...
function so_loadAnimal(){
  ...
  ['p3BucketLand','p3BucketAir','p3BucketWater'].forEach(id=>document.getElementById(id).classList.remove('correct-pick','wrong-pick','disabled'));
...
function so_pickBucket(type){
  if(so_answered)return;so_answered=true;
  ['p3BucketLand','p3BucketAir','p3BucketWater'].forEach(id=>document.getElementById(id).classList.add('disabled'));
  const isCorrect=type===so_current.cat;
  ...
  if(isCorrect){
    document.getElementById(bucketMap[type]).classList.add('correct-pick');
    so_score+=10; saveProgress(ACT_IDS[3],Math.round((so_score/SO_MAX)*100));document.getElementById('p3Score').textContent=so_score;
    document.getElementById('p3Feedback').textContent=c.feedbackCorrect(so_current.name,catName[type]);
    ...
    document.getElementById(sortedMap[type]).appendChild(mini);
    confetti(CAT_COLORS[so_current.cat]);
  } else {
    document.getElementById(bucketMap[type]).classList.add('wrong-pick');
  ...
  else setTimeout(so_loadAnimal,900);
}
...
const Q_COPY={en:{title:'Animal Quiz!',sub:'Answer the questions about animals!',feedbackCorrect:'✅ Correct! Well done!',feedbackWrong:a=>`❌ The answer was ${a}`,resultTitle:'Amazing!',resultSub:'You finished the Animal Quiz!',next:'Next'},tl:{title:'Animal Quiz!',sub:'Sagutin ang mga tanong tungkol sa mga hayop!',feedbackCorrect:'✅ Tama! Napakahusay!',feedbackWrong:a=>`❌ Ang sagot ay ${a}`,resultTitle:'Kahanga-hanga!',resultSub:'Natapos mo ang Animal Quiz!',next:'Susunod'}};
const Q_TOTAL=10,Q_MAX=Q_TOTAL*10,Q_LETTERS=['A','B','C','D'];
let q_pool=[],q_current=null,q_answered=false,q_score=0,q_qIdx=0,q_lives=3;
...
function q_startGame(){
  q_score=0;q_qIdx=0;q_lives=3;q_answered=false;q_pool=q_buildPool();
...
function q_loadQuestion(){
  ...
  document.getElementById('p4QBadge').innerHTML=`<i class="fas fa-paw"></i> `+(typeof q_current.badge==='function'?q_current.badge():q_current.badge);
  const choices=typeof q_current.choices==='function'?q_current.choices():q_current.choices;
  const correctKey=typeof q_current.correct==='function'?q_current.correct():q_current.correct;
  const grid=document.getElementById('p4Choices');grid.innerHTML='';
  choices.forEach((ch,i)=>{
    const isCorrect=ch.key===correctKey;
    ...
    btn.onclick=()=>q_checkAnswer(btn,ch.key,isCorrect,correctKey);
    grid.appendChild(btn);
  });
}
function q_checkAnswer(btn,chosen,isCorrect,correctKey){
  if(q_answered)return;q_answered=true;
  document.querySelectorAll('#p4Choices .choice-btn').forEach(b=>b.classList.add('answered'));
  const c=Q_COPY[lang];
  if(isCorrect){
    btn.classList.add('correct');q_score+=10; saveProgress(ACT_IDS[4],Math.round((q_score/Q_MAX)*100));document.getElementById('p4Score').textContent=q_score;
    ...
    confetti(q_current.color);speak(correctKey,lang);
  } else {
    btn.classList.add('wrong');
    document.querySelectorAll('#p4Choices .choice-btn').forEach(b=>{const t=b.querySelector('.choice-text');if(t&&t.textContent.startsWith(correctKey))b.classList.add('correct');});
    q_lives--;q_updateLives();
    document.getElementById('p4Feedback').textContent=c.feedbackWrong(correctKey);
    document.getElementById('p4Feedback').className='feedback-msg wrong';
  }
  q_qIdx++;
  if(q_qIdx>=Q_TOTAL||q_lives<=0) setTimeout(q_showResult,1100);
  else setTimeout(q_loadQuestion, 1200);
}
...
function flushActiveTabProgress() {
  switch(activeTab) {
    case 1: if(m_score>0) saveProgress(ACT_IDS[1],Math.round((m_score/M_MAX)*100)); break;
